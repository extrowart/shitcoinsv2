...
import { calcDecimalValue, calcNonDecimalValue, deployJsonData, getKeypairFromEnv, sendAndConfirmTransaction, sleep } from "./utils";
...
export async function createToken(input: CreateTokenInput): Promise<Result<{ tokenId: string, txSignature: string }, string>> {
    try {
        const { decimals, name, image, symbol, website, telegram, twitter, url, initialMintingAmount } = input;
        ...
        if (input.image) metadata.image = image
        console.log({ input })
